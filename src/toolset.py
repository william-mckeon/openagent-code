...
from .tools import (TOOLS, WEB_TOOLS, MEMORY_TOOLS, SKILL_TOOLS, PATCH_TOOLS, GOAL_TOOLS, EFFORT_TOOLS,
                    PROPOSE_TOOLS, TODO_TOOLS, SPEC_TOOLS, WORKFLOW_TOOLS, openai_schemas)
...
def active_tools():
    ...
    if config.SPEC_FIRST:
        tools += SPEC_TOOLS
    # The ONLY site that offers run_workflow (Phase 38 / specs/0038) — gated here, not refused at call time,
    # so a flag-off run's logged tool_schemas stay byte-identical (same discipline as every gate above).
    if config.WORKFLOWS:

...
Phase 0 ✅ · Phase 1 ✅ (eval now 13 tasks, 13/13 — harder tier added, ceiling not yet found) · Phase 2 ✅ · Phase 3 ✅ · **Phase 4 — compaction ✅ + subagents ✅ + planning ✅ + interactivity ✅ + tool-breadth ✅ + cold-start handling ✅ + permissions Core ✅ + cross-session memory ✅ + permission hooks ✅ (specs/0015). Remaining: the always-open robustness/eval-ceiling tail** · **Phase 5 — distillation flywheel (teacher = whatever `.env` selects, currently Inkling-Small on Tinker → distilled student): Stage 1 (documented) ✅; Stages 2-7 staged (specs/0005)** · **Adoption Track (Phase 12–19): `0012` situational-context ✅ · `0013` edit-layer ✅ · `0014` auto-verify ✅ · `0015` hooks ✅ · `0016` execpolicy ✅ · `0017` sandbox-FS ✅ · `0019` guardian ✅ · `0020` goal-loop ✅ · `0021` adaptive-effort ✅ · `0022`–`0060` build-out all ✅ (propose-mode + follow-through `0022`/`0048`, workflows `0038`–`0040`, streaming `0043`, reasoning-control `0044`, auto-max-tokens `0045`, shell-hints `0046`, grounding-early-stage `0047`, extra-body `0049`, self-preservation `0050`, prompt-hygiene `0051`, propose-autoplan `0052`, runtime-done `0053`, window-probe-UA `0054`, shell-noninteractive `0055`, runtime-done-broaden `0056`, guardian-interactive `0057`, greenfield-absence `0058`, scrub-trajectory `0059`, effort-pin-precedence `0060`) · remaining: `0018` sandbox-net (admin/WFP, own timeline). Foundation hardened by a full adversarial audit (19/19 fixed) + live rides — see docs/AUDIT-FINDINGS.md**. **Runtime: gpt-oss-120b → Together → Inkling-Small on Tinker (OpenAI-compatible, 256k window); still an env-only swap.**

...
(whatever `.env` selects — **currently Inkling-Small on Tinker**; originally gpt-oss-120b on Bedrock) to **distill a smaller, deployable student**
...
- **0019 guardian** ✅ BUILT (2026-07) — a captured, fail-CLOSED LLM approval-reviewer for the `ask`
...
- **0021 adaptive-effort** ✅ BUILT (2026-07) — match reasoning effort to task difficulty instead of a
  fixed level. Two triggers on one dial (`Model.effort`, re-read every call): the agent self-escalates via
  the `escalate_effort` tool, and the harness auto-escalates on struggle (tool failures / gate re-prompts /
  a failing goal bar). A **pluggable Policy** (`CODE_EFFORT_POLICY`: off | reactive | online | your
  module) makes "how effort is decided" a switch — the deterministic `reactive` default ships in
  `effort.py`, the opt-in self-learning `online` policy is isolated in `effort_online.py`. Escalate-only,
  capped, depth-0, reset per turn. The escalation is captured (`effort_change` + a per-step `effort` field)
  so the flywheel learns the policy. `CODE_ADAPTIVE_EFFORT`, default off; `check_effort` (21) +
  `check_effort_online` (14).
...
Phase 0 ✅ · Phase 1 ✅ (eval now 13 tasks, 13/13 — harder tier added, ceiling not yet found) · Phase 2 ✅ · Phase 3 ✅ · **Phase 4 — compaction ✅ + subagents ✅ + planning ✅ + interactivity ✅ + tool-breadth ✅ + cold-start handling ✅ + permissions Core ✅ + cross-session memory ✅ + permission hooks ✅ (specs/0015). Remaining: the always-open robustness/eval-ceiling tail** · **Phase 5 — distillation flywheel (teacher = whatever `.env` selects, currently Inkling-Small on Tinker → distilled student): Stage 1 (documented) ✅; Stages 2-7 staged (specs/0005)** · **Adoption Track (Phase 12–19): `0012` situational-context ✅ · `0013` edit-layer ✅ · `0014` auto-verify ✅ · `0015` hooks ✅ · `0016` execpolicy ✅ · `0017` sandbox-FS ✅ · `0019` guardian ✅ · `0020` goal-loop ✅ · `0021` adaptive-effort ✅ · `0022`–`0081` build-out all ✅ (propose-mode + follow-through `0022`/`0048`, workflows `0038`–`0040`, streaming `0043`, reasoning-control `0044`, auto-max-tokens `0045`, shell-hints `0046`, grounding-early-stage `0047`, extra-body `0049`, self-preservation `0050`, prompt-hygiene `0051`, propose-autoplan `0052`, runtime-done `0053`, window-probe-UA `0054`, shell-noninteractive `0055`, runtime-done-broaden `0056`, guardian-interactive `0057`, greenfield-absence `0058`, scrub-trajectory `0059`, effort-pin-precedence `0060`, identity-hardening `0061`, context-self-state `0062`, agent-identity-block `0063`, show-reasoning `0064`, read-only-integrity `0065`, identity-scope `0066`, narration-stall `0067`, identity-strip `0068` (withdrawn), narration-quoted-text `0069`, repl-crash-honesty `0070`, security-boundary `0071`, log-review-fixes `0072`, gate-honesty `0073`, resume-integrity `0074`, model-io `0075`, fanout `0076`, train-harness-audit `0077`, secret-exfil `0078`, netfence `0079`, perm-hardening `0080`, execpolicy-hardening `0081`, secrets-at-rest `0082`, os-sandbox `0083` (restricted-token + job-object spawn, validated on Windows), subagent-deadlock-stall `0084` (propose-inheritance deadlock fix + no-progress stall breaker, from the 9f1891 loop forensics), narration-as-final `0085` (ROOT fix: a reply-by-print step ENDS the turn with its text instead of looping), compaction-depoison `0086` (strip a narration loop out of history before summarize/resume), grounding-anti-collapse `0087` (a flaky verifier / collapsing correction no longer turns a review into a "confirmed X" receipt), review-substance `0088` (prompt no longer orders reviews terse + a digest backstop when the weak model collapses a review to a receipt), lean-prompt `0089` (BASE_PROMPT stripped 81% — 9.8k->1.8k chars/turn — behind CODE_LEAN_PROMPT), lean-prompt-pass2 `0090` (secondary prompts: tool descriptions + build_system_prompt notes + PowerShell + trailers — assembled system prompt 18k->9.2k, 48%), subagent-budget `0091` (keep the main agent premium, make spawned children cheap: CODE_SUBAGENT_EFFORT + CODE_SUBAGENT_MAX_STEPS, explicit grounding/guardian effort still wins; armed low/12 with the count knobs MAX_REVIEW_AREAS 16->6 / FANOUT 8->3 / DEPTH 2->1), advisory-register `0092` (CODE_ADVISORY_REGISTER — a research/design thought-partner turn no longer collapses to a status receipt; native_tools_note stops pinning "a short final summary" + an ADVISORY prose-not-receipt note, from the 7170fa4eb4dd Centpilot log), actually-responds `0093` (two default-OFF fixes for the intermittent no-answer on full Inkling, log 98d6cbd9d8a2: CODE_NARRATION_FULL_ANSWER nudges for the complete answer instead of finalizing on a header/status fragment, and CODE_GROUND_ABSENCE_STRICT stops the deterministic absence check firing a phantom self-perpetuating "X is missing/empty" on "add missing code"/action-negations; adversarial-review-hardened: modifier-tolerant absence predicate + multi-line-receipt-aware fragment classifier), shell-hints-extra `0094` (CODE_SHELL_HINTS_EXTRA — one env line for the heredoc / ls-la / which / head-tail footguns the lean block dropped, plus a $env:TEMP scratch-file discipline)) · remaining: `0018` sandbox-net (admin/WFP, own timeline). Prompt inventory (every model-facing string, KEEP/TRIM/CUT) captured; SUMMARIZE/SYNTHESIS + guardian criteria + nudges deferred to a later trim. Foundation hardened by a full adversarial audit (19/19 fixed) + live rides — see docs/AUDIT-FINDINGS.md**. **Runtime: gpt-oss-120b → Together → Inkling-Small on Tinker (OpenAI-compatible, 256k window); still an env-only swap.**

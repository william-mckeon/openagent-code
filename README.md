...
| `CODE_SHOW_REASONING` | `false` | Tee the model's reasoning to the console live (specs/0064): a dimmed "thinking" stream in the top-level REPL; needs `CODE_STREAM`; subagents/eval stay silent |
...
| `CODE_CONTEXT_SELF_STATE` | `false` | Self-state in the situational block (specs/0062): append the current reasoning effort (e.g. `xhigh`) so the agent reports its real level when asked; needs `CODE_SITUATIONAL_CONTEXT` |

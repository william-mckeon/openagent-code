...
def main():
    ...
    deep_calls = []
    ctx_deep = _ctx(tmp, depth=1, spawn=lambda t, effort=None, label=None, **_k: deep_calls.append(t) or "UNGROUNDED: x -> y")
    ...
    check("semantic ON: verifier is the authority (a phantom path it clears -> clean)",
          grounding.problems("I made `docker/ghost.md`.",
                             _ctx(tmp, spawn=lambda t, effort=None, label=None, **_k: "GROUNDED")) == [])
    dir_calls = []
    grounding.problems("Auth is wired via `docker/auth` per the compose.",
                       _ctx(tmp, spawn=lambda t, effort=None, label=None, **_k: dir_calls.append(t) or "GROUNDED"))
    check("semantic ON: a DIRECTORY citation still spawns the verifier (no false-negative)",
          len(dir_calls) == 1)
    calls = []

    def spawn_flag(task, effort=None, label=None, **_k):
        calls.append(task)
    ...
    check("read-only run fires the verifier; a GROUNDED verdict clears it (no over-flag)",
          grounding.problems("This project is documented in `docker/README.md`.",
                             _ctx(tmp, spawn=lambda t, effort=None, label=None, **_k: ro_calls.append(t) or "GROUNDED")) == []
          and len(ro_calls) == 1)
    check("Tier 2 empty/error verdict -> fail-open",
          grounding.semantic_problems("x", {"a.md"}, lambda t, effort=None, label=None, **_k: "") == []
          and grounding.semantic_problems("x", {"a.md"}, lambda t, effort=None, label=None, **_k: "(subagent error: boom)") == [])
    ...
    out = grounding.problems(
        ...
        _ctx(tmp, spawn=lambda t, effort=None, label=None, **_k: abs_calls.append(t) or
    ...
    grounding.problems("Everything looks consistent and well-organized.",   # no path, no absence claim
                       _ctx(tmp, spawn=lambda t, effort=None, label=None, **_k: noabs_calls.append(t) or "GROUNDED"))
    ...
    ac = grounding.problems("The `src/auth/cmd/server/main.go` file is missing.",
                            _ctx(adir, spawn=lambda t, effort=None, label=None, **_k: "GROUNDED"))

...
| `CODE_VERIFY_RUNTIME_DONE` | `false` | Reference — runtime-done honesty (specs/0053, broadened specs/0056): flag a "service is up/serving/plumbed", "<app> runs", "verified running", or "deploy fixed" claim when no health-check (curl/http/port) returned ok this turn; safety-fingerprint: No |
| `CODE_SHELL_NONINTERACTIVE` | `false` | Reference — non-interactive `run_command` (specs/0055): PowerShell `-NonInteractive` + child stdin `DEVNULL`, so a command that reads stdin (bare `echo` / Read-Host) fails fast instead of hanging the REPL; safety-fingerprint: No |
| `CODE_CONTEXT_SELF_STATE` | `false` | Reference — self-state in the situational block (specs/0062): append the current reasoning effort (e.g. `xhigh`) so the agent reports its real level when asked; needs `CODE_SITUATIONAL_CONTEXT`; model id omitted (specs/0061); safety-fingerprint: No |
| `CODE_SCRUB_TRAJECTORY` | `false` | Reference — trajectory PII/secret scrubbing (specs/0059): redact keys/tokens/emails + financial PII (amounts, account IDs, cards) from each persisted record at the write choke point; live context untouched; safety-fingerprint: No |
| `CODE_GUARDIAN_INTERACTIVE` | `false` | Reference — interactive guardian (specs/0057): the AI guardian adjudicates ask-tier commands in the REPL, auto-approving the clearly-safe and deferring the rest to the human `[y/N]`; needs `CODE_GUARDIAN`; safety-fingerprint: No |
...
| `CODE_REPLY_SHAPE` | `false` | Yes (specs/0041) — a user reply-shape/length instruction this turn outranks a tool's "synthesize now" trailer, and is scoped per-turn |

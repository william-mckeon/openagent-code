...
> unbounded and blow the window. Phase C3 of the capability track (ROADMAP.md).
...
The discipline: no unbounded model-visible items; everything injected has a hard cap; oversized items
are flagged. C3 makes that an enforced invariant here rather than an ad-hoc per-caller habit.
...
so an oversized fragment is visible for review, not silent.
...
- **Structured fragment types** (a typed-fragment trait) — our fragments are plain
...
- **Retuning the cap** — 48000 chars is the existing default; C3 enforces the invariant, it does not
  retune. (Our cap is ~12k tokens, configurable via the knob.)
- **Cache-miss avoidance** ("avoid frequent context changes") — a serving-layer
